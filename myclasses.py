...
def on_button_click(root, canvas, day, msg, w, h):
    ...
    if day <= today:
        ...
        #     wraplength=w / 2
        # )
        # canvas.create_window(w/2,h/2,width=w/2,height=h/2,window=newtxt)
...
def get_messages(name):
    ...
    file_id = {'emma': '11SjWgDz9QdPLg7YQzUNkYNzLe8OesAhLEGPUg2ghBU0',
               'pamela': '16kiqK7sfPloITDMybPTA95nISq2-AbLiZXNL12FtNrE',
    ...
    my_messages = ['\n'+m[1].rstrip().replace('\r', '')+'\n' for m in re.findall(pattern, data)]
    my_messages += ['' for _ in range(31 - len(my_messages))]
    return my_messages
